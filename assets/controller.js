...
function updatePrices(container, variant) {
  ...
  const priceContainer = n$2(selectors$P.priceContainer, container);
  const {
    zeroPriceDisplay,
    customPriceContent
...
class Product {
  ...
  _initPhotoCarousel() {
    ...
    const initialSlide = mobileFeaturedImage ? mobileFeaturedImage.dataset.slideIndex : 0;
    import(flu.chunks.swiper).then(_ref => {
      let {
        Swiper,
        Pagination
...
register("quick-cart", {
  onLoad() {
    ...
    }), c("quick-cart:scrollup", () => this.scrollUpQuickCart()), c(["quantity-update:subtract", "quantity-update:add", "quantity-update:change"], (_, _ref2) => {
      let {
        key
      } = _ref2;
...
register("shoppable", {
  ...
  _showWash() {
    u$1(this.wash, classes$6.active);
  },
  _closeDrawer() {
  ...
  _closeAll() {
    ...
    this.hotspotContainers.forEach(spot => i$1(spot, classes$6.active));
    i$1(this.wash, classes$6.active);
  },
  onBlockDeselect() {
    this._closeAll();
  },
...
document.addEventListener('umg:event-handler', function (evt) {
  r$1(...Object.values(evt.detail))
})